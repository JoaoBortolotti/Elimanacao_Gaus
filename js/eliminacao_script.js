	...
	calculator:{
		...
		calculateMatrix: function(mtr, outputId, advancedOutput){
			...
			while(runner+1 < mtr.length){
				advancedOutput && (document.getElementById(outputId).innerText += 'Coluna ' + (runner+1) + ' para 0:\n');
				for (var i = runner+1; i < mtr.length; i++) {
					mtr[i] = gaussian.calculator.calcRows(mtr[runner], mtr[i], runner);
					advancedOutput && (document.getElementById(outputId).innerText += 'Iteração ' + printedStep + ':\n');
					advancedOutput && (document.getElementById(outputId).innerText += 'Subtraindo a oluna ' + (runner+1) + ' de ' + (i+1) + '\n');
